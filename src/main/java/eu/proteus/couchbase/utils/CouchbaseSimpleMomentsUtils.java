...
public class CouchbaseSimpleMomentsUtils {
	...
	public static void createSimpleMomentsFirstTime(String coilID, Object record, ArrayList<String> topicLIst,
		...
		simpleMoments = JsonObject.empty().put("var-id", ((MomentsResult) record).getVarId())
				.put("mean", ((MomentsResult) record).getMean()).put("variance", ((MomentsResult) record).getVariance())
				.put("counter", ((MomentsResult) record).getCounter())
				.put("stdDeviation", ((MomentsResult) record).getStdDeviation())
				.put("x", ((MomentsResult) record).getX()).put("y", ((MomentsResult) record).getY());

		proteusSimpleMoments.add(simpleMoments);

		JsonObject structureProteusDocument = JsonObject.empty()
				.put("coilID", Integer.toString(((MomentsResult) record).getCoilId()))
				.put("values", proteusSimpleMoments);

		StringBuilder documentName = new StringBuilder().append(DOCUMENT_HEADER).append(DOCUMENT_SEPARATOR)
				.append(coilID);

		JsonDocument doc = JsonDocument.create(documentName.toString(), structureProteusDocument);
	...
	public static void updateSimpleMomentsDocument(Bucket proteusBucket, ArrayList<String> topicsList, Object record) {
		...
		proteusBucket.mutateIn(documentName.toString()).arrayAppend("values", simpleMoments).execute();
	}
