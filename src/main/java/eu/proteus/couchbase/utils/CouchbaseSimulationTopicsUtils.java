...
public class CouchbaseSimulationTopicsUtils {
	...
	public static void createSimpleMomentsFirstTime(String coilID, Object record, ArrayList<String> topicLIst,
			Bucket proteusBucket) {

		List<JsonObject> proteusSimpleMoments = new ArrayList<>();
		JsonObject simpleMoments = JsonObject.create();
		simpleMoments = JsonObject.empty().put("coil", ((MomentsResult) record).getCoilId())
				.put("var-id", ((MomentsResult) record).getVarId()).put("mean", ((MomentsResult) record).getMean())
		...
		JsonObject structureProteusDocument = JsonObject.empty().put("simple-moments-coilID", coilID)
				.put("simple-moments", simpleMoments);

		StringBuilder head = new StringBuilder().append("Simple-Moments").append("-").append(coilID);

		JsonDocument doc = JsonDocument.create(head.toString(), structureProteusDocument);
	...
	public static void updateSimpleMomentsDocument(Bucket proteusBucket, ArrayList<String> topicsList, Object record) {
		...
		StringBuilder head = new StringBuilder().append("Simple-Moments").append("-")
		...
		simpleMoments = JsonObject.empty().put("coil", ((MomentsResult) record).getCoilId())
				...
				.put("counter", ((MomentsResult) record).getCounter())
				.put("stdDeviation", ((MomentsResult) record).getStdDeviation());
		proteusBucket.mutateIn(head.toString()).arrayAppend(topicsList.get(0), simpleMoments).execute();
	}

	public static void createDocumentFirstTime(String coilID, Object record, ArrayList<String> topicList,
			Bucket proteusBucket) {
		...
		if (((Measurement) record).getType() == 0x00) {
			row1d = JsonObject.empty().put("position-x", ((Measurement) record).getPositionX())
					.put("value", ((Measurement) record).getValue()).put("var", ((Measurement) record).getVarName());

		}

		if (((Measurement) record).getType() == 0x01) {
			row2d = JsonObject.empty().put("position-x", ((Measurement) record).getPositionX())
					.put("position-y", ((Measurement) record).getPositionY())
					.put("value", ((Measurement) record).getValue()).put("var", ((Measurement) record).getVarName());
		}

		if (record.getClass().equals(HSMMeasurement.class)) {

			hsm = JsonObject.empty().put("variables-counter", ((Measurement) record).getHSMVarCounter())
					.put("variables", ((Measurement) record).getHSMVariables());
	...
	public static void updateDocument(Bucket proteusBucket, ArrayList<String> topicsList, Object record) {
		...
		if (!row1d.isEmpty()) {
			proteusBucket.mutateIn(((Measurement) record).getStringCoilID()).arrayAppend(topicsList.get(0), row1d)
					.execute();
		}

		if (!row2d.isEmpty()) {
			proteusBucket.mutateIn(((Measurement) record).getStringCoilID()).arrayAppend(topicsList.get(0), row2d)
					.execute();
		}

		if (!hsm.isEmpty()) {
			proteusBucket.mutateIn(((Measurement) record).getStringCoilID()).arrayAppend(topicsList.get(0), hsm)
					.execute();
		}
